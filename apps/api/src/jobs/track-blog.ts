...
/**
 * Upsert a single blog article record using onConflictDoUpdate to avoid
 * race conditions when the worker runs with concurrency > 1.
 */
async function upsertBlogArticle(
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: [existingBlogArticles.clientId, existingBlogArticles.url],
      set: {
        title: article.title,
        topics: article.topics,
        keywords: article.keywords,
...
export async function processTrackBlogJob(job: Job<TrackBlogJobData>): Promise<void> {
  ...
  if (!client.websiteUrl) {
    const msg = "Client has no website URL — set a website URL before running blog tracking";
    await db
      .update(agentJobs)
      .set({ status: "failed", errorMessage: msg, completedAt: new Date() })
      .where(eq(agentJobs.id, agentJobId));
